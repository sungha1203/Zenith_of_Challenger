...
// Lighting.hlsl 수정 및 디버깅용 조명 로직 추가
...
float4 Lighting(float3 objectPosition, float3 normal, float3 toEye,
    float4 diffuse, MaterialData material)
{
    float3 output = float3(0.f, 0.f, 0.f);
	
    for (int i = 0; i < MaxLight; ++i)
    {
        if (g_lights[i].type == Last)
            break;
...
float4 ToonLighting(float3 worldPos, float3 normal, float3 toEye, float4 albedoColor)
{
    // g_lights[0]이 디렉셔널 라이트라고 가정
    float3 lightDir = normalize(-g_lights[0].direction);
    ...
    float3 intensity = g_lights[0].strength; // 광원 세기
    return float4(albedoColor.rgb * diffuse * intensity, albedoColor.a);
}
