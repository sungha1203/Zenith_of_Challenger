...
VSOutput VSMain(VSInput input)
{
    ...
    // 가중합 스키닝 포지션 계산
    float4 skinned =
        mul(pos, g_MboneMatrices[input.boneIndices.x]) * weights.x +
        mul(pos, g_MboneMatrices[input.boneIndices.y]) * weights.y +
        mul(pos, g_MboneMatrices[input.boneIndices.z]) * weights.z +
        mul(pos, g_MboneMatrices[input.boneIndices.w]) * weights.w;
