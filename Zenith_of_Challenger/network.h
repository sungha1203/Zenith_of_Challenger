...
public:
	...
	static void		SendInitialState(const std::vector<int> & client_id);			// 다른 플레이어들한테 내 초기 상태
	...
	static void		SendInitMonster(const std::vector<int> & client_id, const std::array<Monster, 50> & monsters);	// 몬스터 초기 좌표 설정
	// ---------패킷 뿌려주기---------
